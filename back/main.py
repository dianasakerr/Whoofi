...
class User(BaseModel):
    ...
    username: str
    email: str
    location: str # as latitude, longitude
    address: str
    city: str
    region: str
    phone_number: int
...
class Dog(BaseModel):
    ...
    def save_user(self):
        ...
        dog_owner_collection, cluster = get_collction_from_db(DOG_OWNER)
        ...
        data = {'name': self.name, 'age': self.age, 'race': self.race, 'owner_id': self.owner_id, 'weight': self.weight,
                'size': self.size}
        collection, cluster = get_collction_from_db(DOG)
...
class DogWalker(User):
    ...
    def save_user(self):
        ...
        data.update({'hourly_rate': self.hourly_rate, 'age': str(self.age),
